...
def parse_questions(filename):
    ...
    for line in lines:
        ...
        if "Teste 2: Vikriti" in line:
            mode = "VIKRITI"
            current_questionnaire_id = 2
            continue
        if "Teste 1: Prakriti" in line:
            mode = "PRAKRITI"
    ...
    # Reset sequences (MySQL Syntax)
    sql_lines.append(f"ALTER TABLE questionario AUTO_INCREMENT = 3;")
    sql_lines.append(f"ALTER TABLE pergunta AUTO_INCREMENT = {question_id};")
    sql_lines.append(f"ALTER TABLE opcao AUTO_INCREMENT = {option_id};")
